...
router.get("/status", async (req: Request, res: Response) => {
  ...
  if (!sub) {
    ...
    const result = GetSubscriptionStatusResponse.parse({
      ...
      analysisCount: 0,
      analysisLimit: null,
      trialRemaining: FREE_TRIAL_DAYS,
      stripeCustomerId: null,
      stripeSubscriptionId: null,
  ...
  try {
    if (event.type === "checkout.session.completed") {
      ...
      if (sessionId && plan) {
        ...
        if (paymentType === "one_time") {
          ...
          await db.update(subscriptionsTable)
            .set({ tier: plan, analysisCount: 0, workoutCount: 0, paymentType: "one_time", currentPeriodEnd, updatedAt: new Date() })
          ...
          await db.update(subscriptionsTable)
            .set({ tier: plan, analysisCount: 0, workoutCount: 0, paymentType: "subscription", stripeSubscriptionId: stripeSubscriptionId ?? undefined, currentPeriodEnd: currentPeriodEnd ?? undefined, updatedAt: new Date() })
            .where(eq(subscriptionsTable.sessionId, sessionId));
        }
    ...
    if (event.type === "invoice.paid") {
      ...
      if (stripeSubscriptionId) {
        ...
        if (sessionId) {
          await db.update(subscriptionsTable)
            .set({ analysisCount: 0, workoutCount: 0, tier: plan ?? "limited", currentPeriodEnd: new Date(stripeSub.current_period_end * 1000), updatedAt: new Date() })
            .where(eq(subscriptionsTable.sessionId, sessionId));
        }
      }
    }
